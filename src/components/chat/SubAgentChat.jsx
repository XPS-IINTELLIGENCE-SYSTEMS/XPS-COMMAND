...
import { Send, GitBranch, Loader2, CheckCircle2, ArrowLeft } from "lucide-react";
import { Button } from "@/components/ui/button";
import { base44 } from "@/api/base44Client";
import ReactMarkdown from "react-markdown";

export default function SubAgentChat({ agent, onStatusChange, onBack }) {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-3 py-2 border-b border-border bg-secondary/30">
        <div className="flex items-center gap-1.5">
          <button onClick={onBack} className="p-0.5 rounded hover:bg-secondary/50 transition-colors" title="Back to main chat">
            <ArrowLeft className="w-3 h-3 text-muted-foreground hover:text-foreground" />
          </button>
