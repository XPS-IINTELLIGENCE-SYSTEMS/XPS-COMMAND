...
import { Users, Package, Hammer, Loader2, RefreshCcw, MapPin, Sparkles, DollarSign, Phone, Mail, X } from "lucide-react";
...
export default function CRMView() {
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      {/* Header */}
      <div className="flex-shrink-0 p-4 md:p-5 space-y-4 border-b border-white/[0.06]">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-extrabold xps-gold-slow-shimmer" style={{ fontFamily: "'Montserrat', sans-serif" }}>CRM PIPELINE</h1>
            <p className="text-sm text-muted-foreground">{filtered.length} leads · ${totalValue.toLocaleString()} value</p>
          </div>
...
function CRMCard({ lead, onClick, index = 0 }) {
  ...
  return (
    <button onClick={onClick}
      className={cn(
        "group flex-shrink-0 w-[270px] md:w-[290px] rounded-2xl p-5 text-left transition-all duration-300 hover:scale-[1.03] animated-silver-border",
        isGlass
