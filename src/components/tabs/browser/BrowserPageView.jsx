...
export default function BrowserPageView({ data, onNavigate, onSearch, onSubmitForm }) {
    ...
    const interceptScript = `
      <script>
        ...
        document.addEventListener('submit', function(e) {
          ...
          var q = fd.get('q') || fd.get('query') || fd.get('search') || '';
          if (q) {
  ...
  useEffect(() => {
    const handler = (e) => {
      if (e.data?.type === 'browser-navigate' && e.data.url) onNavigate(e.data.url);
      else if (e.data?.type === 'browser-search' && e.data.query) onSearch(e.data.query);
    };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", width: "100%", overflow: "hidden", background: "#fff" }}>
      {/* Mode toggle bar */}
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 12px", background: "#f1f3f4", borderBottom: "1px solid #dadce0", flexShrink: 0 }}>
        <button
          onClick={() => setMode("live")}
          style={{
            display: "flex", alignItems: "center", gap: 6, padding: "4px 12px", borderRadius: 9999, fontSize: 12, fontWeight: 500, border: "none", cursor: "pointer",
            background: mode === "live" ? "#fff" : "transparent",
            color: mode === "live" ? "#1a1a1a" : "#6b7280",
            boxShadow: mode === "live" ? "0 1px 2px rgba(0,0,0,0.1)" : "none",
          }}
        >
        ...
        <button
          ...
          style={{
            display: "flex", alignItems: "center", gap: 6, padding: "4px 12px", borderRadius: 9999, fontSize: 12, fontWeight: 500, border: "none", cursor: "pointer",
            background: mode === "data" ? "#fff" : "transparent",
            color: mode === "data" ? "#1a1a1a" : "#6b7280",
            boxShadow: mode === "data" ? "0 1px 2px rgba(0,0,0,0.1)" : "none",
          }}
        >
        ...
        <span style={{ fontSize: 10, color: "#9ca3af", maxWidth: 180, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{data.title}</span>
      </div>
          ...
          <>
            {srcdoc ? (
              <>
                ...
                {data.url?.includes("google.com") && !data.url?.includes("search") && (
                  <div style={{
                    ...
                    padding: "6px 12px", borderRadius: 9999, background: "rgba(0,0,0,0.7)",
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%" }}>
                <p style={{ fontSize: 14, color: "#9ca3af" }}>No visual preview available. Switch to Data view.</p>
              </div>
            )}
          </>
        )}
          ...
          <div style={{ height: "100%", overflowY: "auto" }}>
            <div style={{ background: "#fff", borderBottom: "1px solid #e5e7eb", padding: "16px 24px" }}>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                {data.favicon && <img src={data.favicon} alt="" style={{ width: 20, height: 20, marginTop: 4, borderRadius: 4, flexShrink: 0 }} onError={(e) => e.target.style.display = 'none'} />}
